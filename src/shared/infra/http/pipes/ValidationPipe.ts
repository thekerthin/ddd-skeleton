...
export class ValidationPipe implements PipeTransform {
  ...
  transform(values: any, { metatype }: ArgumentMetadata) {
    ...
    Object.entries(properties).forEach(([propertyName, { valueObject, props }]: [string, any]) => {
      const propertyValue = values[propertyName];
      const valueObjectConstructorProps = Reflect.getOwnMetadata('__constructor_props__', props);

      const valueObjectProps = valueObjectConstructorProps.reduce((params, propName) => {
